...
async function main() {
  ...
  const bqSeeds = [
    ...
    // delta-merge tables
    // Delta tables: use TIMESTAMP_SECONDS(DIV(ms,1000)) to match Hive truncation of CAST(ms/1000 AS BIGINT)
    `INSERT INTO ${BQDS}.${BQP}ods_timesheet SELECT timesheet_id,agent_id,work_date,program_id,billable_minutes,nonbillable_minutes,approved_flag,TIMESTAMP_SECONDS(DIV(change_ms,1000)),LEFT(work_date,7) FROM ${BQDS}.${BQP}stg_fin_timesheet_delta WHERE op='I'`,
    `INSERT INTO ${BQDS}.${BQP}ods_payroll_adjustment SELECT adjustment_id,agent_id,adj_type,amount,TIMESTAMP_SECONDS(DIV(change_ms,1000)),period_month FROM ${BQDS}.${BQP}stg_fin_payroll_adj_delta WHERE op='I'`,
    `INSERT INTO ${BQDS}.${BQP}ods_sla_credit SELECT sla_credit_id,program_id,sla_target_id,credit_amount,reason,TIMESTAMP_SECONDS(DIV(change_ms,1000)),period_month FROM ${BQDS}.${BQP}stg_crm_sla_credit_delta WHERE op='I'`,
    `INSERT INTO ${BQDS}.${BQP}ods_callback_request SELECT callback_id,call_id,queue_id,TIMESTAMP_SECONDS(requested_epoch),TIMESTAMP_SECONDS(scheduled_epoch),completed_flag,TIMESTAMP_SECONDS(DIV(change_ms,1000)),'2024-01-15' FROM ${BQDS}.${BQP}stg_tel_callback_request_delta WHERE op='I'`,
    `INSERT INTO ${BQDS}.${BQP}ods_shift_swap SELECT swap_id,requesting_agent_id,accepting_agent_id,schedule_id,swap_date,status,TIMESTAMP_SECONDS(DIV(change_ms,1000)),LEFT(swap_date,7) FROM ${BQDS}.${BQP}stg_wfm_shift_swap_delta WHERE op='I'`,
    `INSERT INTO ${BQDS}.${BQP}ods_ticket_worklog SELECT worklog_id,ticket_id,agent_id,minutes_logged,TIMESTAMP_SECONDS(DIV(log_ms,1000)),note,TIMESTAMP_SECONDS(DIV(change_ms,1000)),'2024-01-15' FROM ${BQDS}.${BQP}stg_tkt_worklog_delta WHERE op='I'`,
    `INSERT INTO ${BQDS}.${BQP}ods_attrition_event SELECT attrition_event_id,agent_id,TIMESTAMP_SECONDS(notice_epoch),last_day,attrition_type,reason_code,regrettable_flag,TIMESTAMP_SECONDS(DIV(change_ms,1000)),LEFT(last_day,7) FROM ${BQDS}.${BQP}stg_hr_attrition_event_delta WHERE op='I'`,
